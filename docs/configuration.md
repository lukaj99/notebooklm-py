...
| `NOTEBOOKLM_MCP_TRUSTED_ACCESS_EMAILS` | Comma-separated owner addresses. When set, the remote OAuth consent page authenticates from the `Cf-Access-Authenticated-User-Email` header instead of prompting for `NOTEBOOKLM_MCP_OAUTH_PASSWORD`. Requires a loopback `NOTEBOOKLM_MCP_HOST` and `NOTEBOOKLM_MCP_PROXY_SHARED_SECRET`. | - |
| `NOTEBOOKLM_MCP_PROXY_SHARED_SECRET` | Secret the fronting proxy must send as `X-Auth-Gate-Secret` on every proxied request. The identity header above is ignored without it, so a proxy that stops stripping client-supplied headers fails closed rather than trusting whatever a caller sends. Required whenever `NOTEBOOKLM_MCP_TRUSTED_ACCESS_EMAILS` is set. | - |
| `NOTEBOOKLM_MCP_AUTO_APPROVE_REDIRECT_URIS` | Comma-separated callbacks that may be approved without a click once the proxy has proven the owner identity. `*` stands in for the port of a loopback host (`http://localhost:*/callback`). Needed because claude.ai registers a fresh `client_id` on every connect, so `OAUTH_AUTO_APPROVE` (which matches on `client_id`) never applies to it. Empty → every new client sees the consent page. | - |
