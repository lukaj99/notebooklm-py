# AGENTS.md — notebooklm-py

Unofficial async Python client for Google NotebookLM. Drives Google's internal `batchexecute` RPC protocol. > Symlinked as CLAUDE.md. Single source of truth for AI agents.

Also follow the file/naming conventions in [CONTRIBUTING.md](CONTRIBUTING.md).
...
Put pure logic in `tests/unit/`, REST adapter coverage in `tests/server/`, VCR-backed flows in `tests/integration/`, and authenticated NotebookLM coverage in `tests/e2e/`. Name tests `test_<behavior>.py`. Coverage is expected to stay at or above the configured 90% threshold.
...
## For Parallel Agents (Codex / multi-agent)
